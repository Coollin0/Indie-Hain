...
class ProfilePage(QWidget):
    ...
    def _build_ui(self):
        ...
        local_sep = QFrame()
        local_sep.setFrameShape(QFrame.HLine)
        profile_lay.addWidget(local_sep)

        local_title = QLabel("Lokale Daten")
        local_title.setStyleSheet("font-size: 16px; font-weight: 600;")
        profile_lay.addWidget(local_title)

        local_grid = QGridLayout()
        local_grid.setHorizontalSpacing(8)
        local_grid.setVerticalSpacing(6)

        self.local_api_lbl = QLabel("")
        self.local_api_lbl.setTextInteractionFlags(Qt.TextSelectableByMouse)
        self.local_path_lbl = QLabel("")
        self.local_path_lbl.setTextInteractionFlags(Qt.TextSelectableByMouse)
        self.local_install_lbl = QLabel("")
        self.local_install_lbl.setTextInteractionFlags(Qt.TextSelectableByMouse)

        local_grid.addWidget(QLabel("API Basis:"), 0, 0)
        local_grid.addWidget(self.local_api_lbl, 0, 1)
        local_grid.addWidget(QLabel("Datenordner:"), 1, 0)
        local_grid.addWidget(self.local_path_lbl, 1, 1)
        local_grid.addWidget(QLabel("Installationsordner:"), 2, 0)
        local_grid.addWidget(self.local_install_lbl, 2, 1)
        profile_lay.addLayout(local_grid)

        local_btn_row = QHBoxLayout()
        self.btn_open_data = QPushButton("Datenordner öffnen")
        self.btn_open_data.setCursor(Qt.PointingHandCursor)
        self.btn_open_installs = QPushButton("Installationsordner öffnen")
        self.btn_open_installs.setCursor(Qt.PointingHandCursor)
        self.btn_change_installs = QPushButton("Installationsordner ändern")
        self.btn_change_installs.setCursor(Qt.PointingHandCursor)
        self.btn_reset_local = QPushButton("Lokale Daten zurücksetzen")
        self.btn_reset_local.setCursor(Qt.PointingHandCursor)
        local_btn_row.addWidget(self.btn_open_data)
        local_btn_row.addWidget(self.btn_open_installs)
        local_btn_row.addWidget(self.btn_change_installs)
        local_btn_row.addWidget(self.btn_reset_local)
    ...
    def _change_install_dir(self):
        ...
        if not new_path.is_absolute():
            new_path = data_root() / new_path
        new_path = new_path.resolve()
        if new_path == current.resolve():
            QMessageBox.information(self, "Installationsordner", "Der Installationsordner bleibt unverändert.")
            return

        try:
            new_path.mkdir(parents=True, exist_ok=True)
        except Exception as exc:
            QMessageBox.warning(self, "Installationsordner", f"Ordner konnte nicht erstellt werden:\n{exc}")
            return

        try:
            add_legacy_install_dir(current)
            update_settings({"install_dir": str(new_path)})
        except Exception as exc:
            QMessageBox.warning(self, "Installationsordner", f"Speichern fehlgeschlagen:\n{exc}")
            return

        self._update_local_info()
        env_override = os.environ.get("INDIE_HAIN_INSTALL_DIR") or os.environ.get("INSTALL_DIR")
        if env_override:
            QMessageBox.information(
                self,
                "Installationsordner",
                "Neuer Installationsordner gespeichert.\n"
                "Hinweis: Eine gesetzte Umgebungsvariable überschreibt die Einstellung.\n"
                "Entferne INDIE_HAIN_INSTALL_DIR/INSTALL_DIR, damit die Änderung greift.",
            )
        else:
            QMessageBox.information(
                self,
                "Installationsordner",
                "Neuer Installationsordner gespeichert.\n"
                "Bestehende Installationen im alten Ordner bleiben verfügbar.",
            )
    ...
    def _reset_local_data(self):
        confirm = QMessageBox.question(
            ...
            "Lokale Daten (Session, Warenkorb, Library) wirklich löschen?\nInstallierte Spiele bleiben erhalten.\nDu wirst ausgeloggt.",
        ...
        try:
            migration_block_path().write_text("skip", encoding="utf-8")
        except Exception:
            pass
        ...
        for path in db_files:
            try:
                ...
                QMessageBox.warning(self, "Lokale Daten", f"Konnte Datei nicht löschen: {path}\n{exc}")

        try:
            store.init_db()
        except Exception as exc:
            QMessageBox.warning(self, "Lokale Daten", f"Neuaufbau der lokalen DB fehlgeschlagen: {exc}")
